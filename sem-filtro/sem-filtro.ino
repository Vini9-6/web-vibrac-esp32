...
void setup() {
  ...
  // Inicializa MPU6050 com alta sensibilidade
  if (mpu.begin()) {
    mpuOk = true;
    mpu.setAccelerometerRange(MPU6050_RANGE_2_G);   // ±2G (0.000061g/LSB)
    mpu.setGyroRange(MPU6050_RANGE_250_DEG);
    mpu.setFilterBandwidth(MPU6050_BAND_260_HZ);    // Largura de banda máxima
    calibrateMPU6050(); // Calibração inicial
...
void loop() {
  ...
  if (client) {
    ...
    if (mpuOk) {
      ...
      // Filtro passa-alta para vibrações
    ...
    // Resposta JSON com dados filtrados
    ...
    // Página HTML com gráficos otimizados + botão e contador
    client.println("HTTP/1.1 200 OK");
...
<script>
  ...
  const chartX = new Chart(ctxX, {
    type: 'line',
    data: { datasets: [{ label: 'Vibração X', borderColor: 'red', data: [] }] },
    options: chartOptions
  });

  const chartY = new Chart(ctxY, {
    type: 'line',
    data: { datasets: [{ label: 'Vibração Y', borderColor: 'green', data: [] }] },
    options: chartOptions
  });

  const chartZ = new Chart(ctxZ, {
    type: 'line',
    data: { datasets: [{ label: 'Vibração Z', borderColor: 'blue', data: [] }] },
    options: chartOptions
  });
  ...
  // Lógica do botão e contador para download dos dados
